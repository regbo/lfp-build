...
object Utils {
    ...
    val tomlMapper: TomlMapper by lazy {
        TomlMapper().apply { registerModules(Jdk8Module(), KotlinModule.Builder().build()) }
    }
    ...
    /**
     * Resolves arbitrary inputs (paths, files, patterns, resources) into readable [Resource]
     * instances. Supports:
    ...
    fun resources(vararg locations: Any?, walkDirectories: Boolean = true): List<Resource> {
        ...
        fun anyToResources(input: Any?): List<Resource> =
            when (input) {
                null -> emptyList()
    ...
    /**
     * Renders an object into a short string form using reflection-based [ToStringBuilder] with
    ...
    /**
     * Converts the input string to a [Regex]. If wrapped in `/.../`, uses it as raw regex;
     * otherwise escapes it for exact matching.
     */
    fun toRegex(input: String): Regex {
        return if (
            input.length > 2 && Stream.of(0, input.length - 1).allMatch { input[it] == '/' }
        ) {
...
private class TrimmedList private constructor(private val delegate: List<String>) :
    ...
    companion object {
        /**
         * Creates a [TrimmedList] from any list, removing null/blank elements and trimming
         * whitespace.
         */
        fun from(list: List<String?>): TrimmedList {
            return if (list is TrimmedList) {
                ...
                val trimmed =
                    list.mapNotNull { it?.trim()?.takeIf { trimmed -> trimmed.isNotEmpty() } }
                TrimmedList(trimmed)
            }
        }
    }
